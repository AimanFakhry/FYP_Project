...
<head>
    ...
    <style>
        ...
        .theme-techy {
            ...
            --panel-border: 1px solid rgba(16, 185, 129, 0.4);
            --text-main: #e2e8f0;
            --text-accent: #10b981;
            --input-bg: #1e293b;
            --input-border: #334155;
            ...
            background-image: linear-gradient(rgba(15, 23, 42, 0.9), rgba(15, 23, 42, 0.9)), repeating-linear-gradient(0deg, transparent, transparent 1px, rgba(16, 185, 129, 0.1) 1px, rgba(16, 185, 129, 0.1) 2px);
            background-size: 100% 4px;
        }
        .theme-spacy {
            --bg-color: #1a0033;
            --panel-bg: rgba(45, 27, 78, 0.85);
            --panel-border: 1px solid rgba(167, 139, 250, 0.5);
            --text-main: #f3f4f6;
            --text-accent: #d8b4fe;
            --input-bg: #2e1065;
            --input-border: #5b21b6;
        ...
        body { background-color: var(--bg-color); color: var(--text-main); font-family: var(--font-family); transition: all 0.3s ease; }
        
        .hud-panel { background-color: var(--panel-bg); border: var(--panel-border); backdrop-filter: blur(12px); box-shadow: 0 4px 6px -1px rgba(0, 0, 0, 0.1); border-radius: 1rem; }
        .hud-input { background-color: var(--input-bg); border: 1px solid var(--input-border); color: var(--text-main); }
        .hud-input:focus { border-color: var(--text-accent); outline: none; box-shadow: 0 0 0 3px rgba(var(--text-accent), 0.2); }
        .text-accent { color: var(--text-accent); }
        .bg-accent { background-color: var(--text-accent); }
        
        /* Fixed Avatar & Theme Selection Styling */
        .avatar-option:checked + div {
        ...
        .theme-option:checked + div {
            border-color: var(--text-accent);
            box-shadow: 0 0 10px var(--text-accent);
            background-color: var(--input-bg);
        }

        /* CRITICAL FIX: Force SVGs to fit inside the container and not overflow */
        .avatar-box svg {
            width: 100% !important;
...
<body class="antialiased min-h-screen theme-{{ $user->theme }}" id="app-body">
    ...
    <main class="pt-28 pb-16">
        <div class="container mx-auto px-4 max-w-4xl">
            ...
            <div class="hud-panel p-8 shadow-2xl">
                <div class="flex items-center justify-between mb-8 border-b border-gray-700/20 pb-4">
                    <h1 class="text-3xl font-bold tracking-tight">Configure Profile</h1>
                    <div class="px-3 py-1 rounded border border-gray-500/30 text-xs font-mono opacity-70">
                ...
                <form action="{{ route('profile.update') }}" method="POST">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                        ...
                        <div class="space-y-6">
                            <div>
                                <label class="block text-sm font-bold uppercase tracking-wider mb-2 opacity-80">Codename</label>
                                <input type="text" name="name" value="{{ old('name', $user->name) }}" 
                                       class="hud-input w-full p-3 rounded-lg font-bold">
                            </div>

                            <div>
                                <label class="block text-sm font-bold uppercase tracking-wider mb-2 opacity-80">Visual Interface</label>
                                <div class="grid grid-cols-1 gap-3">
                                    <label class="cursor-pointer group relative">
                                        <input type="radio" name="theme" value="techy" class="sr-only theme-option" {{ $user->theme == 'techy' ? 'checked' : '' }}>
                                        <div class="p-3 rounded-lg border-2 border-transparent hover:border-gray-500/50 transition-all flex items-center gap-3">
                                            <div class="w-8 h-8 rounded bg-[#0f172a] border border-[#10b981] flex items-center justify-center text-[#10b981] text-xs font-mono">>_</div>
                                            <span class="font-bold">Techy</span>
                                        </div>
                                    </label>

                                    <label class="cursor-pointer group relative">
                                        <input type="radio" name="theme" value="spacy" class="sr-only theme-option" {{ $user->theme == 'spacy' ? 'checked' : '' }}>
                                        <div class="p-3 rounded-lg border-2 border-transparent hover:border-gray-500/50 transition-all flex items-center gap-3">
                                            <div class="w-8 h-8 rounded bg-[#1a0033] border border-[#d8b4fe] flex items-center justify-center text-[#d8b4fe] text-xs">★</div>
                                            <span class="font-bold">Spacy</span>
                                        </div>
                                    </label>

                                    <label class="cursor-pointer group relative">
                                        <input type="radio" name="theme" value="cheerful" class="sr-only theme-option" {{ $user->theme == 'cheerful' ? 'checked' : '' }}>
                                        <div class="p-3 rounded-lg border-2 border-transparent hover:border-gray-500/50 transition-all flex items-center gap-3">
                                            <div class="w-8 h-8 rounded bg-white border border-[#f472b6] flex items-center justify-center text-[#db2777] text-xs">:)</div>
                                            <span class="font-bold">Cheerful</span>
                                        </div>
                                    </label>
                                </div>
                            </div>
                        ...
                        <!-- Right Column: Avatar -->
                        <div>
                            <label class="block text-sm font-bold uppercase tracking-wider mb-4 opacity-80 text-center">Select Avatar</label>
                            ...
                            <div class="grid grid-cols-3 gap-4">
                             ...
                             @endphp
                                ...
                                @foreach($avatars as $key => $svg)
                                    <label class="cursor-pointer group flex flex-col items-center">
                                        <input type="radio" name="avatar" value="{{ $key }}" class="sr-only avatar-option" {{ $user->avatar == $key ? 'checked' : '' }}>
                                        <div class="avatar-box w-20 h-20 rounded-full border-4 border-transparent flex items-center justify-center p-2 transition-all hover:scale-105 overflow-hidden" 
                                             style="background-color: var(--input-bg); color: var(--text-main);">
                                        ...
                                        <p class="text-center text-xs mt-2 font-bold uppercase opacity-60 group-hover:opacity-100">{{ ucfirst($key) }}</p>
                                    </label>
                                @endforeach
                            </div>
                        </div>
                    ...
                    <div class="flex justify-end gap-4 mt-8 pt-6 border-t border-gray-700/20">
                        <a href="{{ route('profile.show') }}" class="px-6 py-3 rounded-lg font-bold border border-gray-500/50 hover:bg-white/10 transition-colors">
                            Cancel
                        </a>
                        <button type="submit" class="px-8 py-3 rounded-lg font-bold text-white bg-accent hover:opacity-90 transition-opacity shadow-lg">
                            Save Changes
                        </button>
                    </div>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            themeInputs.forEach(input => {
                input.addEventListener('change', (e) => {
                    ...
                    body.classList.remove('theme-cheerful', 'theme-spacy', 'theme-techy');
