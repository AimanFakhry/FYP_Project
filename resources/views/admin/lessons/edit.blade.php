        ...
        <div class="container mx-auto px-4 max-w-4xl">
            <div class="bg-white shadow-sm rounded-lg border border-gray-200 overflow-hidden">
                <div class="px-6 py-4 border-b border-gray-200 bg-gray-50 flex justify-between items-center">
                    <div>
                        <h1 class="text-lg font-bold text-gray-900">Edit Lesson</h1>
                        <p class="text-xs text-gray-500 mt-1">ID: {{ $lesson->id }}</p>
                    </div>
                    <a href="{{ route('admin.courses.show', $lesson->lessonGroup->course) }}" class="text-sm text-gray-600 hover:text-gray-900">Cancel</a>
                </div>
                    ...
                    <form action="{{ route('admin.lessons.update', $lesson) }}" method="POST">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
                            <div class="md:col-span-2">
                                ...
                                <input type="text" name="title" id="title" required value="{{ $lesson->title }}"
                            ...
                            <div>
                                ...
                                <input type="number" name="order" id="order" required value="{{ $lesson->order }}"
                                       class="block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm py-2 px-3 border">
                            </div>
                        </div>
                        ...
                        <div id="intro-content-section" class="mb-6">
                            ...
                            <textarea name="content" id="content" rows="3" 
                                      class="block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm py-2 px-3 border">{{ $lesson->content }}</textarea>
                        </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                            ...
                            <div>
                                ...
                                <select name="activity_type" id="activity_type" class="block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm py-2 px-3 border bg-gray-50">
                                    <option value="text_only" {{ $lesson->activity_type == 'text_only' ? 'selected' : '' }}>Text Only</option>
                                    <option value="fill_in_the_blank" {{ $lesson->activity_type == 'fill_in_the_blank' ? 'selected' : '' }}>Fill in the Blank</option>
                                    <option value="sandbox" {{ $lesson->activity_type == 'sandbox' ? 'selected' : '' }}>Code Sandbox</option>
                                </select>
                            </div>
                            <!-- Achievement -->
                            <div>
                                ...
                                <select name="achievement_id" class="block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm py-2 px-3 border">
                                    ...
                                    @foreach($achievements as $achievement)
                                        <option value="{{ $achievement->id }}" {{ $lesson->achievement_id == $achievement->id ? 'selected' : '' }}>
                                            {{ $achievement->name }}
                                        </option>
                                    @endforeach
                                </select>
                        ...
                        <div id="section-fill_in_the_blank" class="activity-section hidden">
                            ...
                            <div class="bg-yellow-50 p-4 rounded-md border border-yellow-200">
                                <div class="mb-4">
                                    ...
                                    <textarea name="fib_question" rows="3" class="block w-full rounded-md border-yellow-300 shadow-sm focus:border-yellow-500 focus:ring-yellow-500 sm:text-sm py-2 px-3 border font-mono">{{ $lesson->fillInTheBlank->question ?? '' }}</textarea>
                                </div>
                                <div>
                                    <label class="block text-sm font-medium text-yellow-800 mb-1">Correct Answer</label>
                                    <input type="text" name="fib_answer" value="{{ $lesson->fillInTheBlank->answer ?? '' }}" class="block w-full rounded-md border-yellow-300 shadow-sm focus:border-yellow-500 focus:ring-yellow-500 sm:text-sm py-2 px-3 border">
                                </div>
                            </div>
                        </div>
                        ...
                        <div id="section-sandbox" class="activity-section hidden">
                            ...
                            <div class="bg-slate-900 p-4 rounded-md border border-slate-700">
                                ...
                                <textarea name="sandbox_code" id="sandbox_code" rows="10" 
                                          ...
                                          spellcheck="false">{{ $lesson->sandbox->starting_code ?? '' }}</textarea>
