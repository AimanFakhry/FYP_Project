...
<head>
    <meta charset="utf-8">
    <title>Edit Achievement - Admin</title>
        ...
        <div class="container mx-auto px-4 max-w-2xl">
            <div class="bg-white shadow-sm rounded-lg border border-gray-200 overflow-hidden">
                <div class="px-6 py-4 border-b border-gray-200 bg-gray-50 flex justify-between items-center">
                    <div>
                        <h1 class="text-lg font-bold text-gray-900">Edit Achievement</h1>
                        <p class="text-xs text-gray-500 mt-1">ID: {{ $achievement->id }}</p>
                    </div>
                ...
                <div class="p-6">
                    <form action="{{ route('admin.achievements.update', $achievement) }}" method="POST">
                        ...
                        <div class="mb-5">
                            ...
                            <input type="text" name="name" id="name" required value="{{ $achievement->name }}"
                        ...
                        <div class="mb-5">
                            ...
                            <textarea name="description" id="description" rows="3" required 
                                      class="block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm py-2 px-3 border">{{ $achievement->description }}</textarea>
                        </div>
                        ...
                        <div class="mb-6">
                            ...
                            <select name="course_id" id="course_id" class="block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm py-2 px-3 border bg-white">
                                ...
                                @foreach($courses as $course)
                                    <option value="{{ $course->id }}" {{ $achievement->course_id == $course->id ? 'selected' : '' }}>
                                        {{ $course->name }} (Uses Course Icon)
                                    </option>
                                @endforeach
                            </select>
